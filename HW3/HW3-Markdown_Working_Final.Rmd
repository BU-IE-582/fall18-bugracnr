...
pathplotter <- function(instanceno)
{
  ...
  #Plot
  scatterplot3d(xx,xy,xz, color = cols, pch = shapes, main = paste("Gesture ", xtrain[instanceno,1]), xlab = "X Axis", ylab = "Y Axis", zlab = "Z Axis")
  legend("topleft", cex = 0.4, legend = c("Start", "Body", "End"), col = c("Blue", "Black", "Red"), pch = c(2,1,4))
}
```
...
pathplotter(11)
```
The plot above shows the first Gesture. When it is compared to original gesture 1, it looks mirrored against Y Axis.

```{r}
pathplotter(20)
```
The plot above shows the second gesture. It is mirrored against Y Axis like the first gesture.
...
The plot above shows the 3rd gesture, and the mirroring problem continues here as well.

```{r}
pathplotter(5)
```

Same issue can be observed in the 4th Gesture's plot.
...
5th gesture is plotted above. This time the mirroring problem didn't occur, however, the direction of the gesture is not exactly vertical. It is distorted in the direction of x=y line (in positive direction). Movement in Z Axis is as expected. 


```{r}
pathplotter(1)
```
The figure above shows the 6th gesture. The movement in the Z Axis is as expected, however, the change in the X and Y axes made the movement S-shaped.


```{r}
pathplotter(7)
```
7th gesture is plotted above. This time the circler movement is as expected, however, the shape is mirrored against Y Axis.

```{r}
pathplotter(6)
```
Finally, I plotted the 8th gesture. The gesture is circler as expected. However, the mirroring problem continues here as well. We can try solve the problem by mirroring the images against Y axis. The correcting function and sample plots for the first two gestures are shown below.

```{r, eval=TRUE, include = TRUE}
pathplotter2 <- function(instanceno)
{
  ax <- unlist(xtrain[instanceno,2:ncol(xtrain)])
  ay <- unlist(ytrain[instanceno,2:ncol(xtrain)])
  az <- unlist(ztrain[instanceno,2:ncol(xtrain)])
...
As we can see from the results above, Model using Euclidean Distance best performs with k = 3, with an accuracy of 94.7%.
...
As it is shown in the table above, the manhattan distance model best performs with k = 1, with an accuracy of 95.8%.
...
As we can see, the euclidean model works with 94.3% accuracy which is a very high value. The accuracy is also very close to the predicted accuracy in cross validation.
...
The manhattan model's accuracy is even better than the first one. The value is 95.4%, which is very close to the result in cross validation.
...
Results for training data. Prediction accuracy of the model with training data is 96%, which is a very high accuracy.
...
We can see the accuracy and confusion matrix of the model for the test data. Accuracy is 88%, which is very high. Even tough the accuracy on the training data is 96%, we can see that the model does not overfits, as the test result are very high as well. Both classes has same number of wrong guesses.
...
As we can see in the figure above, the regression coefficients correspond to huge changes in the movements of ECG plots. Where the plots are somehow more stabile, the coefficient are zero or very close to zero. In the areas where the movements are higher, the values of the coefficients respond them. Also there are 38 nonzero coefficients in the model, and the optimal lambda values are 2.13 & 1.03.
...
The table above shows the confusion matrix for the training data. The performance on training data is 90% which is high but not as high as the original model. The model predicts more false negatives, then false positives. I used the terms negative and positive corresponding to the original values of classes (-1 & 1).
...
The results for the test data are shown above. The performance of the model on test data is 86%, which is not as high as the original models' performance (88%). However, while original model has a 8% difference between training and test performance, this model has 4% difference. The similar performances on the training and test data may be interpreted as the latter model is more robust, and behaves as expected. Additinally, false negative and false positive number distribution is also like the result on the training data.
...
When we look at the graph we can see nonzero coefficients are collected around close indexes. However, the second model has less nonzero elements then the first one. The nonzero elements are also larger in the second model. The number of nonzeroes are 18, and lambdas are 1.04 & 2.22. The plot below shows both of the plots for comparison. 
